...
def test_login_user_not_found():
    user = {"email": "testlogindoesnotexist@mail.com", "password": "12"}
    response = client.post("/login", json=user)
...
def test_login_password_incorrect():
    user = {"email": "testlogin@mail.com", "password": "1"}
    response = client.post("/login", json=user)
    assert "error" in response.json()
...
def test_user_delete_authorized():
    ...
    response = client.delete(
        "/", json=user, headers={"Authorization": f"Bearer {token}"}
    )
    assert response.status_code == 200
